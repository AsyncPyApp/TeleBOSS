...
def help_msg(message):
    ...
    utils.bot.reply_to(message, "Список всех доступных команд для СисУпАн текущей версии:\n\n"
                                "/adduser [сообщение] – заявка на вступление нового пользователя\n"
                                "/kickuser – голосование за мут пользователя\n"
                                "/banuser – голосование за бан пользователя\n"
                                "/threshold [2..кол-во участников или auto] [ban] – изменить пороги голосов за пункт "
                                "для принятия решения при достижении значения\n "
                                "/timer [5..86400] [ban] – изменить время таймера, по достижению которого решение "
                                "принимается автоматически\n "
                                "/delmsg – голосование за удаление сообщения\n"
                                "/clearmsg – голосование за бесследное удаление сообщения\n"
                                "/op – дать админку реплейнутому участнику по голосованию, "
                                "изменить звание себе или реплейнутому боту\n"
                                "/deop [me] – снять админку с себя или по голосованию с участника\n"
                                "/abuse [1..12] (Обоюдоострый Меч) – взаимный мут двух <s>баранов</s> пользователей, "
                                "последний аргумент уровня ядерной войны. Снимает админки автоматом, "
                                "автоматически после снятия не возвращает\n"
                                "/mute – голосование за мут участника\n"
                                "/unmute /unban – голосование за разбан/размут участника\n"
                                "/whitelist – проверить, входит ли реплейнутый участник в белый список\n"
                                "/getchat (только в режиме пре-инициализации) - получить ID чата\n"
                                "/getuser (только в режиме отладки) - получить ID пользователя\n"
