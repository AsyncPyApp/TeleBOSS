...
_MIGRATED_CALLERS: tuple[str, ...] = (
    ...
    "src/teleboss/app/host_commands/membership.py",
    "src/teleboss/app/host_commands/info.py",
    "src/teleboss/app/host_commands/moderation.py",
    "src/teleboss/app/host_commands/misc.py",
    "src/teleboss/app/host_commands/__init__.py",
    "src/teleboss/voting/bases.py",
    "src/teleboss/voting/engine.py",
    "src/teleboss/domain/moderation/prevote_join.py",
    "src/teleboss/domain/admin/prevote_op.py",
    "src/teleboss/domain/moderation/prevote_messages.py",
)
...
def _sql_worker_public_method_names() -> set[str]:
    ...
    ``SqlWorker`` is composed from mixins under ``src/teleboss/shared/storage/``;
    method bodies no longer all live in the facade module.
    """
    storage_dir = REPO_ROOT / "src/teleboss/shared/storage"
    ...
    facade = REPO_ROOT / "src/teleboss/shared/storage/sql_worker.py"
...
def test_migrated_callers_may_use_rem_rec_for_cleanup() -> None:
    ...
    assert "src/teleboss/app/handlers/votes.py" in found
    assert "src/teleboss/voting/bases.py" in found
...
def test_displayed_poll_lookups_use_get_open_poll_with_two_args() -> None:
    ...
    targets = {
        "src/teleboss/app/handlers/votes.py": "call_msg_chk",
        "src/teleboss/app/host_commands/membership.py": "add_answer",
        "src/teleboss/domain/moderation/prevote_messages.py": "pre_return",
    }
...
def test_duplicate_and_timer_paths_use_unique_id_lookup() -> None:
    ...
    for rel, func_name in (
        ("src/teleboss/voting/bases.py", "is_voting_exist"),
        ("src/teleboss/domain/moderation/prevote_join.py", "is_voting_exist"),
        ("src/teleboss/domain/admin/prevote_op.py", "is_voting_exist_op"),
        ("src/teleboss/voting/engine.py", "vote_result"),
    ):
        path = REPO_ROOT / rel
        tree = ast.parse(path.read_text(encoding="utf-8"))
        fn = next(
...
def test_engine_timer_and_restart_pass_unique_id_to_vote_result() -> None:
    ...
    path = REPO_ROOT / "src/teleboss/voting/engine.py"
